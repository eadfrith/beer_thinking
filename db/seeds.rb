...
CSV.foreach(Rails.root.join("brewing_methods.csv"), headers: true) do |row|
...
CSV.foreach(Rails.root.join("ww_kits.csv"), headers: true) do |row|
...
CSV.foreach(Rails.root.join("bjcp_beers.csv"), headers: true) do |row|
